...
class TopoCleanup : public GeomBasics
{
	/** A dart used when traversing the mesh in cleanup operations. */
private:
	Dart* d;
	int count = 0;
	std::vector<Element*> deleteList;
...
private:
	...
	//             new node
	//
	Dart* fill3( Quad* q, Edge* e, Node* n, bool safe );
	...
	//
	Dart* fill4( Quad* q, Edge* e, Node* n2 );
	...
	///
	void applyComposition( Dart* startDart, const std::vector<uint8_t>& fillPat );
	...
	inline static std::vector<uint8_t> comp4 = { 5, // Consider 4 instead of 3... nah, 3 is best here
			0, 3, 0, 3 };
	...
	inline static std::vector<uint8_t> comp5 = { 13, // Consider 4 instead of 3...
	...
	void addNodes( const std::vector<Node*>& arrayOfNodes, int len );
...
private:
	...
	///
	Dart* getDartAt( Node* c, const std::vector<Node*>& neighbors, int i );
	...
	///
	Dart* closeQuad( Quad* q, Edge* e1, Node* nK, bool centroid );
	...
	///
	Dart* openQuad( Quad* q, Edge* e, Node* n1 );
	...
	///
	Dart* switchDiagonalCCW( Quad* qa, Edge* e1a, Node* n );
	...
	// @return a dart representing the input dart after the operation is performed.
	///
	Dart* switchDiagonalCW( Quad* qa, Edge* e1a, Node* n );
